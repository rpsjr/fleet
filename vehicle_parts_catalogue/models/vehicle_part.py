...
class VehiclePart(models.Model):
    ...
    vehicle_model_id = fields.Many2one(
        "fleet.vehicle.model", string="Vehicle Model", required=True
    )
    inspection_item_id = fields.Many2one(
        "fleet.vehicle.inspection.item", string="Inspection Item", required=True
    )
    name = fields.Char(
        related="vehicle_model_id.name", store=True, readonly=True, string="Model Name"
    )
    part_number = fields.Char(string="Código da Peça", required=True)
    qty = fields.Float(string="Quantidade Aplicada", digits=(16, 2), default=1.0)
