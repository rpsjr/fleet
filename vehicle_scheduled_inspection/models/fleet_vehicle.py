...
class FleetVehicle(models.Model):
    ...
    def _cron_schedule_inspections(self):
        ...
        for vehicle in vehicles:
            ...
            plans = self.env['fleet.vehicle.model.inspection.plan'].search([('model_id', '=', vehicle.model_id.id)])
            if not plans:
                continue
            ...
            for plan in plans:
                ...
                last_km = last_line.inspection_id.odometer if last_line and last_line.inspection_id.odometer else 0.0
            ...
            if items_7d:
                ...
                inspection = self.env['fleet.vehicle.inspection'].create({
                    ...
                    'inspection_line_ids': [(0, 0, {'item_id': item.id}) for item in unique_items]
                })

                inspection.message_subscribe(partner_ids=[vehicle.driver_id.id])

                vehicle._send_whatsapp_invite(inspection, unique_items, forecast_7d, inspection_date)
                vehicle._send_email_invite(inspection, unique_items, forecast_7d, inspection_date)

    def _send_whatsapp_invite(self, inspection, items, forecast_km, date):
        self.ensure_one()
        template = self.env.ref('vehicle_scheduled_inspection.wa_template_convite_manutencao_programada', raise_if_not_found=False)
        if not template:
            return
        ...
        v5 = "✅" if match_item(["óleo", "motor"]) or match_item(["oleo", "motor"]) else "❌"
        v6 = "✅" if match_item(["filtro", "óleo"]) or match_item(["filtro", "oleo"]) else "❌"
        v7 = "✅" if match_item(["filtro", "combustível"]) or match_item(["filtro", "combustivel"]) else "❌"
        v8 = "✅" if match_item(["filtro", "ar"]) else "❌"
        
        known_items = ["óleo", "oleo", "filtro de óleo", "filtro de oleo", "filtro de combustível", "filtro de combustivel", "filtro de ar"]
        v9 = "✅" if any(not any(k in n for k in known_items) for n in item_names) else "❌"
    ...
    def _send_email_invite(self, inspection, items, forecast_km, date):
        self.ensure_one()
        template = self.env.ref('vehicle_scheduled_inspection.mail_template_convite_manutencao_programada', raise_if_not_found=False)
        if template:
